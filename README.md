...
- **후속 댓글 지원**: Issue에 달린 추가 질문에도 자동 응답 (관리자 댓글은 무시)
...
2. 이슈 카테고리 판별 (Bug Report / Enhancement Request / Firewall Request / Questions)
...
자동 무시 대상:
- `codemate` (bot 계정) 댓글 → 무한 루프 방지
- `config.ini`에 등록된 관리자 댓글 → 관리자 응답에 중복 답변 방지

### 배포 절차

#### 1. config.ini 수정

`config.ini`에 실제 GitHub 사용자 ID를 등록 (관리자 겸 자동 배정 대상):
...
| `DOCS_BASE_URL` | Docs 사이트 URL |
| `BOT_PAT` | Bot 계정(`codemate`)의 Personal Access Token |

#### 3. Bot 계정 권한 설정

`codemate` 계정을 Repository → Settings → Collaborators에서 **Write** 권한으로 추가.
(Issue 댓글 작성, assignee 배정, label 추가에 필요)
...
Push 후, Issue가 생성되면 자동으로 VOC Bot이 응답합니다.

## Issue 카테고리

Bot은 4가지 카테고리의 Issue를 처리합니다:
